...
void mapper_000(Cartridge* cart, Cpu6502* CPU, PPU_Struct* p)
{
	...
	if (cart->prg_rom.size == 16 * KiB) {
		memcpy(&CPU->RAM[0x8000], cart->prg_rom.data, 16 * KiB); // First 16KiB
		memcpy(&CPU->RAM[0xC000], cart->prg_rom.data, 16 * KiB); // Last 16KiB (Mirrored)
	} else {
		memcpy(&CPU->RAM[0x8000], cart->prg_rom.data, 32 * KiB);
	}
