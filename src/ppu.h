...
void ppu_reset(int start, Ppu2C02* p, Cpu6502* cpu); /* Emulates reset/warm-up of PPU */

/* Debug Functions */
void debug_ppu_regs(Cpu6502* cpu);
void ppu_mem_16_byte_viewer(Ppu2C02* PPU, unsigned start_addr, unsigned total_rows);
void OAM_viewer(Ppu2C02* PPU, enum PpuMemoryTypes ppu_mem); // rename to VRAM viewer?
